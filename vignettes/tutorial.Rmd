...
```{r}
# 載入但不顯示標籤
pos_tagger <- load_tagger_pos("pos-fast", 
                              show_tags = FALSE)
```
...
<!-- tagger <- flair_models()$SequenceTagger$load("flair/ner-english-ontonotes") -->
<!-- Sentence <- flair_data()$Sentence -->
...
<!-- vapply(sentence, function(e) e$get_labels(), character(1)) -->

<!-- vapply(sentence$get_labels(), function(x) x$value, character(1)) -->
<!-- vapply(sentence$get_spans(), function(e) e$text, character(1)) -->
<!-- vapply(sentence$labels, function(label) label$score,  numeric(1))   -->


<!-- vapply(sentence$labels, function(e) e$score, character(1)) -->
<!-- apply(sentence$get_labels(), function(label) { -->
<!--     label$value -->
<!-- }, character(1)) -->
...
<!-- ### Tagging Sentiment Across Full DataFrame -->

<!-- <div style="text-align: justify;"> -->

<!-- Load the pre-trained model "`sentiment`". The pre-trained models of "`sentiment`", "`sentiment-fast`", and "`de-offensive-language`" are currently available. For more pre-trained models, see <https://flairnlp.github.io/docs/tutorial-basics/tagging-sentiment>. -->
...
<!-- ```{r} -->
<!-- results <- get_sentiments(texts = uk_immigration$text, -->
<!--                           doc_ids = uk_immigration$speaker,  -->
<!--                           tagger = tagger_sent) -->
<!-- head(results, n = 10) -->
<!-- ``` -->
...
BERT embeddings provide powerful contextual representations through their bidirectional transformer architecture. These embeddings are good at understanding context from both directions within text, generating deep contextual representations through multiple transformer layers, and leveraging pre-training on large text corpora to achieve strong performance across NLP tasks. 

The classic BERT base model generates 768-dimensional embeddings for each token, providing rich semantic representations.
...
Dynamic embeddings, such as BERT and Flair, adapt their word representations based on context using high-dimensional vector spaces (BERT uses 768 dimensions in its base model). These methods excel at handling contextual nuances: BERT employs self-attention mechanisms and subword tokenization, while Flair uses character-level modeling. Both effectively handle out-of-vocabulary words through these mechanisms.

Static embeddings, like FastText (300 dimensions) and GloVe (typically 100-300 dimensions), maintain consistent word representations regardless of context. FastText stands out among static embeddings by incorporating subword information, giving it an advantage in handling out-of-vocabulary words, while GloVe relies on pre-computed global word-word co-occurrence statistics.

Each embedding method has its unique characteristics and trade-offs. While they share the common goal of capturing meaningful textual relationships, they differ in their balance of contextual awareness, computational demands, and representation stability. The choice among them depends on specific project requirements, including accuracy needs, computational resources, and the nature of the text being analyzed.
