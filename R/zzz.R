...
#' FlaiR: R Interface to Flair NLP
#'
#' @description
#' FlaiR provides an R interface to the Flair NLP library, enabling natural
#' language processing tasks in R using Python's Flair library.
#'
#' @name flaiR
#' @aliases flaiR-package
NULL
...
#' Package environment
#'
#' Internal environment for storing package state
#'
#' @name .pkgenv
#' @keywords internal
...
#' Package constants
#'
#' Version requirements and other constants
#'
...
.pkgenv$package_constants <- list(
  ...
  numpy_max_version = "1.28.99",  # Below 1.29.0 for SciPy compatibility
...
#' Module storage
#'
#' Storage for imported Python modules
#'
#' @name .modules
#' @keywords internal
.pkgenv$modules <- list(
...
#' Handle environment conflicts
#'
#' @keywords internal
#' @noRd
handle_env_conflicts <- function() {
  .pkgenv$original_env <- list(
...
restore_env <- function() {
  if (!is.null(.pkgenv$original_env)) {
    if (.pkgenv$original_env$python != "") {
      Sys.setenv(RETICULATE_PYTHON = .pkgenv$original_env$python)
    }
    if (.pkgenv$original_env$virtualenv != "") {
      Sys.setenv(VIRTUALENV = .pkgenv$original_env$virtualenv)
    }
  }
}
...
check_conda_env <- function(show_status = FALSE) {
  ...
  conda_available <- tryCatch({
    ...
    if (show_status) {
      print_status("Conda", conda_bin, TRUE)
    }
    TRUE
  }, error = function(e) {
    FALSE
  ...
  if (conda_available) {
    ...
    if ("flair_env" %in% conda_envs$name) {
      python_path <- conda_envs$python[conda_envs$name == "flair_env"][1]
      if (file.exists(python_path)) {
        if (show_status) {
          packageStartupMessage(sprintf("Using environment: %s", python_path))
        }
        tryCatch({
...
#' @param extra_message Optional extra message
#' @keywords internal
#' @noRd
...
#'
#' @param venv Virtual environment name
...
install_dependencies <- function(venv = NULL, max_retries = 3, quiet = FALSE) {
  version_in_range <- function(version, min_ver, max_ver) {
    tryCatch({
      as.numeric_version(version) >= as.numeric_version(min_ver) &&
        as.numeric_version(version) < as.numeric_version(max_ver)
    }, error = function(e) FALSE)
  }

  check_numpy_version <- function() {
    numpy <- reticulate::import("numpy")
    numpy_version <- reticulate::py_get_attr(numpy, "__version__")

    if (!version_in_range(numpy_version,
                          .pkgenv$package_constants$numpy_min_version,
                          .pkgenv$package_constants$numpy_max_version)) {
      warning(sprintf(
        "NumPy version %s is not in the supported range (%s - %s)",
        numpy_version,
        .pkgenv$package_constants$numpy_min_version,
        .pkgenv$package_constants$numpy_max_version
      ))
  ...
  tryCatch({
    packages <- list(
      numpy = sprintf("numpy>=%s,<%s",
                      .pkgenv$package_constants$numpy_min_version,
                      .pkgenv$package_constants$numpy_max_version),
      scipy = sprintf("scipy==%s",
                      .pkgenv$package_constants$scipy_version),
      torch = sprintf("torch>=%s",
                      .pkgenv$package_constants$torch_version),
      transformers = sprintf("transformers==%s",
                             .pkgenv$package_constants$transformers_version),
      flair = sprintf("flair>=%s",
                      .pkgenv$package_constants$flair_min_version)
    )
    ...
    for (pkg in names(packages)) {
      ...
      for (i in 1:max_retries) {
        tryCatch({
          if (!quiet && i > 1) {
            packageStartupMessage(sprintf("Retry %d/%d for %s",
                                          i, max_retries, pkg))
          }
          ...
          if (i == max_retries) {
            stop(sprintf("Failed to install %s after %d attempts",
                         pkg, max_retries))
          }
          Sys.sleep(2 ^ i)
        })
    ...
    if (!quiet) {
      packageStartupMessage(sprintf("Error installing dependencies: %s",
                                    e$message))
    }
    return(FALSE)
  })
...
#' Package initialization (continued)
#'
#' @keywords internal
...
.onAttach <- function(libname, pkgname) {
  ...
  tryCatch({
    ...
    if (!check_conda_env()) {
      packageStartupMessage("Failed to initialize Python environment")
    ...
    # Format Python status with version and path
    print_status("Python", python_version, check_python_version(python_version))
    packageStartupMessage(sprintf("Python path: %s", python_path))
    packageStartupMessage("")

    # Initialize required modules
    init_result <- initialize_modules()
    if (!init_result$status) {
      packageStartupMessage(sprintf("Module initialization failed: %s",
                                    init_result$error))
      return(invisible(NULL))
    }
    ...
    packageStartupMessage(sprintf("Error during initialization: %s", e$message))
  }, finally = {
