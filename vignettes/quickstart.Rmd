---
title: "Quick Start"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Quick Start}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r include=FALSE}
#reticulate::use_condaenv(reticulate::py_config()[[1]])
reticulate::py_install("flair")
```

## Install `flaiR` with Using [`remotes`](https://github.com/r-lib/remotes):

<div style="text-align: justify">


To utilize flairR in R, you need to first install the Python-based Flair NLP library. This can be done using reticulate. It's recommended to have Python version 3.7 or higher installed to ensure smooth operation of [`fliar`](https://flairnlp.github.io) in Python. The functionality of flairR is primarily limited to the pre-trained models provided by [`fliar`](https://flairnlp.github.io). The `flairR` returns the tagging information directly in a data.table format. Note that using the transformed base NLP toolkit can be computationally intensive. Therefore, implementing parallel computing is advised to enhance the performance of NLP tasks.


<div style="text-align: center; border: 1px solid;">
  <img src="https://raw.githack.com/davidycliao/flaiR/main/man/figures/ent.png" width="800" />
</div>


&nbsp;


</div>

```
> install.packages("remotes")
> remotes::install_github("davidycliao/flaiR", force = TRUE)
> library(flaiR)
 flaiR: An R Wrapper for Accessing Flair NLP Tagging Features      
 Python : 3.11                                           
```

&nbsp;

-----


## First-Time User Installation  

<div style="text-align: justify">

`flaiR` is built on top of the [`reticulate`](https://rstudio.github.io/reticulate/) package and incorporates key features of the Flair library, returning data in a tidy and efficient[`data.table`](https://cran.r-project.org/web/packages/data.table/index.html) format. The installation consists of two parts: First, install [Python 3.7](https://www.python.org/downloads/) or higher, and the[`reticulate`](https://rstudio.github.io/reticulate/) package in R. Additionally, you'll also need [`Anaconda`](Anaconda) to assist `reticulate` in setting up your Python environment, as well as enabling your RStudio to identify the environment.

If you're using `reticulate` with R for the first time, you may encounter some issues. For instance, if your R environment cannot locate your Python configuration, I recommend that you create a new Python environment using `use_condaenv()`. After that, you can install the Python Flair library by using py_install("flair"). 

</div>

<div style="text-align: justify">

If this is your first time using Python-related packages in R, __it's highly likely that you haven't yet installed a Conda environment.__ The function `flaiR::create_flair_env()` `flaiR` in will guide you through the process of installing a Conda environment named `r-reticulate`.  Then, you can directly install the Python Flair library using `py_install()` from within R.

</div>


```{r eval=FALSE, include=TRUE}
py_install("flair")
```

<div style="text-align: justify">

Another scenario is when your R session is already linked to a Python environment. 

</div>

```
> flaiR::create_flair_env()
Environment creation stopped.
Flair is already installed in /Library/Frameworks/Python.framework/Versions/3.11/bin/python3
Using Flair:  0.12.2   
````
<div style="text-align: justify">

If you don't have a specific preference for renaming Python environment, you can directly install the Python Flair library using `py_install()` from within R. Please note, frequently switching Python environments in RStudio may lead to issues or errors. It's recommended to restart RStudio after changing environments.

</div>

&nbsp;

-----

## Quick Use

### __Tagging Parts-of-Speech with Flair Models__

<div style="text-align: justify">

Load the pretrained model "pos-fast". For more pretrained models, see https://flairnlp.github.io/docs/tutorial-basics/part-of-speech-tagging#-in-english. 

</div>

```{r}
library(reticulate)
library(flaiR)
texts <- c("UCD is one of the best universities in Ireland.",
           "UCD has a good campus but is very far from my apartment in Dublin.",
           "Essex is famous for social science research.",
           "Essex is not in the Russell Group, but it is famous for political science research and in 1994 Group.",
           "TCD is the oldest university in Ireland.",
           "TCD is similar to Oxford.")

doc_ids <- c("doc1", "doc2", "doc3", "doc4", "doc5", "doc6")
tagger_pos = import("flair.nn")$Classifier$load('pos-fast')
results <- get_pos(texts, doc_ids, tagger_pos)
head(results, n = 10)
```


### __Tagging Entities with Flair Models__

<div style="text-align: justify">

Load the pretrained model "ner". For more pretrained models, see https://flairnlp.github.io/docs/tutorial-basics/tagging-entities.

</div>

```{r}
library(reticulate)
library(data.table)
tagger_ner = import("flair.nn")$Classifier$load('ner')
results <- get_entities(texts, doc_ids, tagger_ner)
head(results, n = 10)
```

### __Tagging Sentiment__

<div style="text-align: justify">
Load the pretrained model "`sentiment`".  The pre-trained models of "`sentiment`", "`sentiment-fast`", and "`de-offensive-language`" are currently available. For more pretrained models, see https://flairnlp.github.io/docs/tutorial-basics/tagging-sentiment.

</div>

```{r}
library(reticulate)
library(data.table)
tagger_sent = import("flair.nn")$Classifier$load('sentiment')
results <- get_sentiments(texts, doc_ids, tagger_sent)
head(results, n = 10)
```

&nbsp;

-----


## How to Contribute

<div style="text-align: justify">

Contributions in the form of comments, code suggestions, and tutorial examples are very welcome. Please note that the `flaiR` is released with the [Contributor Code of Conduct](https://github.com/davidycliao/flaiR/blob/master/CONDUCT.md). By contributing to this project, you agree to abide by its terms.

</div>




