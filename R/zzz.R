...
.onAttach <- function(...) {
  ...
  # Check Python path
  if (python_path == "") {
    packageStartupMessage("Cannot locate Python executable. Ensure it's installed and in your system's PATH. flaiR functionality requiring Python will not be available.")
  ...
  tryCatch({
    python_version <- system(paste(python_path, "--version"), intern = TRUE)
    if (!grepl("Python 3", python_version)) {
      packageStartupMessage("Python 3 is required. flaiR functionality requiring Python will not be available.")
      return(invisible(NULL))
    }
  }, error = function(e) {
    packageStartupMessage(paste("Failed to get Python version. Error:", e$message))
    return(invisible(NULL))
  })
  ...
  if (isFALSE(flair_version[[2]])) {
    ...
    # Docker-specific installation commands
    if (in_docker) {
      commands <- c(
