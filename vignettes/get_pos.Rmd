...
library(flaiR)
data("de_immigration")
de_immigration <- head(de_immigration, 2)
```

Download the de-pos part-of-speech tagging model from FlairNLP on Hugging Face.
```{r}
tagger_pos <- load_tagger_pos("de-pos")
```

```{r}
results <- get_pos(de_immigration$text, 
...
## Part-of-Speech Tagging in Batch Processing
```{r}
batch_process_results  <- get_pos_batch(de_immigration$text,
                                        de_immigration$speaker, 
                                        tagger_pos, 
                                        show.text_id = FALSE,
                                        batch_size = 2,
                                        verbose = TRUE)
